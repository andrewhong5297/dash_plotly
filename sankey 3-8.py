...
# maximum of 6 value cols -> 6 colors
    ...
    #auto size graphic based on number of nodes
...
app.layout = html.Div([
    ...
    html.Label('Select Path for Breakdown (be sure to clear hover before loading)'),
    dcc.Dropdown(
    ...
    html.Label('Select Business Area (be sure to clear hover before loading)'),
    dcc.Dropdown(
    ...
    html.Label('Can we add a percent flow hover, as well as commentary box? also maximum change detection as default for dumbell plot dropdown'),
...
def update_figure(path,select,hov):#month,day): 
    if len(select)!=0:
        ...
        if len(select)==2:
            filtered_df=df[(df["neighbourhood"]==select[0]) | (df["neighbourhood"]==select[1])] #change to list interpreator
    else:
        filtered_df=df
    ...
    if len(path)>1:
        ...
        if len(path)>2:
            ...
            if len(path)>3:
                d = path[3]
                return genSankey(filtered_df,cat_cols=[a,b,c,d],value_cols='number_of_reviews',title='Number of Reviews',hover = hov)
            return genSankey(filtered_df,cat_cols=[a,b,c],value_cols='number_of_reviews',title='Number of Reviews', hover = hov)
        return genSankey(filtered_df,cat_cols=[a,b],value_cols='number_of_reviews',title='Number of Reviews', hover = hov)
    return genSankey(filtered_df,cat_cols=[a],value_cols='number_of_reviews',title='Number of Reviews', hover = hov)
